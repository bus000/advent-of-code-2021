...
handleInput steps = print . mulPos . foldl step initState $ steps
  where
    initState = SubState 0 0 0

step :: SubState -> Command -> SubState
step (SubState x y aim) (Forward n) = SubState (x + n) (y + aim * n) aim
step (SubState x y aim) (Down n) = SubState x y (aim + n)
step (SubState x y aim) (Up n) = SubState x y (aim - n)

mulPos :: SubState -> Int
mulPos (SubState x y _) = x * y
