...
data Direction = East | South deriving (Show, Ord, Eq)

type Herd = Set.Set (Int, Int)
...
handleInput :: SeaFloor -> IO ()
handleInput = print . stepsToConverge

stepsToConverge :: SeaFloor -> Int
stepsToConverge sf = oneOff + 1
  where
    oneOff = length . takeWhile (uncurry (/=)) $ zip sfs (tail sfs)
    sfs = iterate step sf
...
moveEast sf = sf { _eastHerd = east' }
  where
    east' = Set.map (move sf East) $ _eastHerd sf

moveSouth :: SeaFloor -> SeaFloor
moveSouth sf = sf { _southHerd = south' }
  where
    south' = Set.map (move sf South) $ _southHerd sf

move :: SeaFloor -> Direction -> (Int, Int) -> (Int, Int)
move sf direction pos
    | floorOccupied sf pos' = pos
    | otherwise = pos'
  where
    pos' = nextPosition sf direction pos

floorOccupied :: SeaFloor -> (Int, Int) -> Bool
floorOccupied (SeaFloor east south _) pos =
    Set.member pos east || Set.member pos south

nextPosition :: SeaFloor -> Direction -> (Int, Int) -> (Int, Int)
nextPosition (SeaFloor _ _ (maxCol, _)) East (col, row) =
    ((col + 1) `mod` maxCol, row)
nextPosition (SeaFloor _ _ (_, maxRow)) South (col, row) = 
    (col, (row + 1) `mod` maxRow)

newFloor :: [[Maybe Direction]] -> SeaFloor
newFloor rows = SeaFloor easts souths lowerRight
  where
    easts = Set.fromList $ do
        (rowN, row) <- zip [0..] rows
        (colN, Just East) <- zip [0..] row
        return (colN, rowN)
    souths = Set.fromList $ do
        (rowN, row) <- zip [0..] rows
        (colN, Just South) <- zip [0..] row
        return (colN, rowN)
    rightestRight = length . head $ rows
    lowestLow = length rows
    lowerRight = (rightestRight, lowestLow)

parseInput :: T.Text -> Either P.ParseError SeaFloor
parseInput = P.parse (parseSeaFloor <* P.eof) ""

parseSeaFloor :: P.Parsec T.Text () SeaFloor
parseSeaFloor = newFloor <$> parseRow `P.endBy` P.newline

parseRow :: P.Parsec T.Text () [Maybe Direction]
parseRow = P.many1 $ P.choice
    [ P.char 'v' *> pure (Just South)
    , P.char '>' *> pure (Just East)
    , P.char '.' *> pure Nothing
    ]
