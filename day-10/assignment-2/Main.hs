...
buildStack :: [Char] -> Either String (Stack Char)
buildStack = L.foldl' f (Right [])
  where
    f (Left err) _  = Left err
    f (Right stack) '('  = Right ('(':stack)
    f (Right stack) '['  = Right ('[':stack)
    f (Right stack) '{'  = Right ('{':stack)
    f (Right stack) '<'  = Right ('<':stack)
    f (Right ('(':stack)) ')' = Right stack
    ...
    f (Right ('{':stack)) '}' = Right stack
    f (Right ('<':stack)) '>' = Right stack
    f _ c = Left $ "Unexpected " ++ [c]

computeScore :: Stack Char -> Int
computeScore = L.foldl' f 0
  where
    f score '(' = score * 5 + 1
    f score '[' = score * 5 + 2
    f score '{' = score * 5 + 3
    f score '<' = score * 5 + 4
    f _ c = error $ "Unexpected char " ++ [c]
