...
handleInput :: [T.Text] -> IO ()
handleInput = print . sum . map syntaxErrorScore

syntaxErrorScore :: T.Text -> Int
syntaxErrorScore text = case parseChunks text of
    Right _ -> 0
    Left err -> analyzeError err

analyzeError :: P.ParseError -> Int
analyzeError err
    | "unexpected \")" `L.isInfixOf` text = 3
    | "unexpected \"]" `L.isInfixOf` text = 57
    | "unexpected \"}" `L.isInfixOf` text = 1197
    | "unexpected \">" `L.isInfixOf` text = 25137
    | otherwise = 0
  where
    text = show err

parseChunks :: T.Text -> Either P.ParseError ()
parseChunks = P.parse (P.many parseChunk *> P.eof *> return ()) ""

parseChunk :: P.Parsec T.Text () ()
parseChunk = P.choice parens
  where
    parens =
        [ P.between (P.char '(') (P.char ')') (P.many parseChunk *> return ())
        , P.between (P.char '[') (P.char ']') (P.many parseChunk *> return ())
        , P.between (P.char '{') (P.char '}') (P.many parseChunk *> return ())
        , P.between (P.char '<') (P.char '>') (P.many parseChunk *> return ())
        ]

parseInput :: T.Text -> Either P.ParseError [T.Text]
parseInput = P.parse (parseLines <* P.eof) ""

parseLines :: P.Parsec T.Text () [T.Text]
parseLines = parseLine `P.endBy` P.newline

parseLine :: P.Parsec T.Text () T.Text
parseLine = T.pack <$> P.many1 (P.choice parens)
  where
    parens = map P.char "()[]{}<>"
