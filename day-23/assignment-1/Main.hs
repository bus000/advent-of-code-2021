...
handleInput burrow = case findSolution burrow of
    Just (_, price) -> print price
...
reachableStates burrow = Dijkstra.dijkstra burrow move
...
pathUnits (Path start middle end) = sum . map pairUnits $ pairs
  where
    ...
    pairUnits (Room _, Room _) = 1
    pairUnits (Hallway R1, Hallway R2) = 1
    pairUnits (Hallway R2, Hallway R1) = 1
    pairUnits (Hallway L1, Hallway L2) = 1
    pairUnits (Hallway L2, Hallway L1) = 1
    pairUnits _ = 2
...
parseBurrow = do
    ...
    return . Burrow . Map.fromList $
        [ (Room AmberTop, a1)
        , (Room BronzeTop, a2)
        , (Room CopperTop, a3)
        , (Room DesertTop, a4)
        , (Room AmberBottom, a5)
        , (Room BronzeBottom, a6)
        , (Room CopperBottom, a7)
        , (Room DesertBottom, a8)
        ]
  where
    skipLine = P.many (P.oneOf ".# ") >> P.newline

parseAmphipod :: P.Parsec T.Text () Amphipod
